...
themes <- read_csv("dissertation-themes-raw-count.csv")
```
...
### Heatmap in {ggplot2}, Viridis colors, Scaled but Unordered

```{r, echo=FALSE, fig.width=16, fig.height=9}
rescale <- function(x) {x * 20 / max(x, na.rm = TRUE)}

themes_to_plot_unordered <-
        themes_data %>%
        rownames_to_column(var = "theme") %>%
        mutate(across(Amelia:Wallace, rescale)) %>%
...
### Heatmap in {ggplot2}, Viridis colors, Scaled and Computationally-sorted 
